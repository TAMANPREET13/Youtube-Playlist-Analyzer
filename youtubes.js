...
const waitTillHTMLRendered = async(pages, timeout = 10000) => {
    ...
    while (checkCounts++ <= maxChecks) {
        let html = await pages.content();
        let currentHTMLSize = html.length;

        let bodyHTMLSize = await pages.evaluate(() => document.body.innerHTML.length);
        ...
        await pages.waitFor(checkDurationMsecs);
    }
};